...
reload=False
if reload:
    res=np.load('../re/reader_influenza_vs_covid.npy')
    pre=res[:,-3:-1].astype(np.float)
    pre=pre/pre.sum(1,keepdims=True)
    pre=pre[:,1]+np.random.randn(pre.shape[0])*0.1
    gt=res[:,-1].astype(np.float)
    name=res[:,0]
    a=np.stack([name,pre,gt],-1)
    np.save('temp_influenza.npy',a)
else:
    a=np.load('temp_influenza.npy')
    name=a[:,0]
    pre = np.array(a[:, 1]).astype(np.float)
    gt = np.array(a[:, 2]).astype(np.float)
...
plt.title('ROC Curve of Distinguishing COVID from Influenza')
...
READER_E=np.zeros(50)
answer = open('../key_result/list/reader_influenza_vs_covid.list', 'r')
answer=answer.readlines()
gt=[1-int('AB-in' in an.split(',')[0]) for an in answer]
answer=[da.split(',')[0] for da in answer]
name_r=np.array(answer)
for id in range(1,6):
    workbook=xlrd.open_workbook("reader1"+str(id)+".xlsx")
    ...
    gt=np.array(gt)
    READER_E+=(gt!=(pre>0.5))
    reader_error_name=name_r[gt!=(pre>0.5)]
...
#plt.xlim([0.0,0.18])
#plt.ylim([0.75,0.98])
axes.legend(['AI system','reader 1','reader 2','reader 3','reader 4','reader 5'],loc='lower right')
plt.savefig('jpgs/roc_influenza_covid.jpg',bbox_inches='tight')

plt.show()
xx=np.where(READER_E>=5)
yy=np.where(READER_E==0)
reader_error_name=name_r[xx]
