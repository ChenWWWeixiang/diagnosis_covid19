...
with open(args.output_file,'w') as f:
    ...
    for a_res in ress:
        ...
        plt.plot(fpr, tpr, label='Non-pneumonia, AUC={:.4f}'.format(np.mean(SAUC[:, 0])))
        fpr, tpr, thresholds = metric.roc_curve(y_one_hot[:,1], norm_x[:,1])
        plt.plot(fpr, tpr, label='CAP, AUC={:.4f}'.format(np.mean(SAUC[:, 1])))
        fpr, tpr, thresholds = metric.roc_curve(y_one_hot[:,2], norm_x[:,2])
        plt.plot(fpr, tpr, label='Influenza, AUC={:.4f}'.format(np.mean(SAUC[:, 2])))
        fpr, tpr, thresholds = metric.roc_curve(y_one_hot[:,3], norm_x[:,3])
        plt.plot(fpr, tpr, label='COVID, AUC={:.4f}'.format(np.mean(SAUC[:, 3])))
